...
import { breadcrumbJsonLd, faqJsonLd, vipPricingJsonLd } from "@/seo/json-ld";
...
type PricingPageProps = {
  searchParams: Promise<{ access?: string; serverId?: string }>;
};
...
export default async function PricingPage({ searchParams }: PricingPageProps) {
  ...
  const renewTarget: VipShopRenewTarget | null =
    accessParam && isVipAccessType(accessParam)
      ? {
          accessType: accessParam,
  ...
  const defaultOffers = (
    allRetakesEnabled
      ? catalog.allRetakes.durations
      : (catalog.servers[0]?.durationOptions ?? [])
  )
  ...
  return (
    <div>
      ...
      {defaultOffers.length > 0 ? (
        <JsonLd id="ld-pricing-offers" data={vipPricingJsonLd(defaultOffers)} />
      ) : null}
        ...
        <Container>
          {session ? (
            ...
            <div className="mb-8 flex flex-col gap-4 rounded-xl border border-border bg-card/70 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <a
                href="/api/auth/steam?returnTo=/pricing"
